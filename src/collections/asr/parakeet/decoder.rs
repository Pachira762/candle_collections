use crate::collections::common::tensor_ext::TensorExt;
...
impl RnntDecoder {
    ...
    pub fn predict(&self, token: u32, state: &mut StackedLstmState) -> Result<Tensor> {
        let x = if token == 1024 {
        ...
        let y = self.dec_rnn.forward(&x, state)?;
        let y = y.transpose(0, 1)?;

        Ok(y)
    }

    pub fn initial_state(&self) -> Result<StackedLstmState> {
        self.dec_rnn.initial_state()
    }
}

pub type StackedLstmState = Vec<LSTMState>;
...
impl StackedLstm {
    ...
    fn forward(&self, xs: &Tensor, state: &mut StackedLstmState) -> Result<Tensor> {
