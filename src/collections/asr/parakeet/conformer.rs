use crate::collections::{
    asr::parakeet::attention::RelPositionMultiHeadAttention,
    common::{
        conv::{Conv1d, Conv1dConfig},
        tensor_ext::TensorExt,
    },
};
use candle::{D::Minus1, Module, ModuleT, Result, Tensor};
use candle_nn::{
...
impl ConformerConvolution {
    pub fn new(
        ...
        let pointwise_conv1 = Conv1d::new(config, vb.pp("pointwise_conv1"))?;
        ...
        let pointwise_conv2 = Conv1d::new(config, vb.pp("pointwise_conv2"))?;
        ...
        let depthwise_conv = CausalConv1D::new(config, conv_context_size, vb.pp("depthwise_conv"))?;
...
impl CausalConv1D {
    fn new(config: Conv1dConfig, padding: (usize, usize), vb: VarBuilder) -> Result<Self> {
        ...
        let config = Conv1dConfig { padding, ..config };
        let conv = Conv1d::new(config, vb)?;
