...
use crate::collections::{
    asr::common::mel::{MelFilterType, MelProcessor, WindowFunc},
...
impl AudioToMelSpectrogramPreprocessor {
    ...
    /// \[T\] => \[B, T/100, F\]
    pub fn forward(&mut self, audio: &[f32]) -> Result<Tensor> {
    ...
    fn normalize(&mut self, x: &Tensor) -> Result<Tensor> {
        ...
        let feats_mean = (x.sum_keepdim(1)? / seq_len)?;
        let feats_std = x
            .broadcast_sub(&feats_mean)?
            .powf(2.0)?
            .sum_keepdim(1)?
            .scalar_div(seq_len - 1.0)?
            .sqrt()?
            .scalar_add(f64::EPSILON)?;
        let x = x.broadcast_sub(&feats_mean)?.broadcast_div(&feats_std)?;
